        ...
        public void fit(List<List<double>> X, List<List<double>> Y, double learningRate, double momentum, int nb_epochs)
        {
            ...
            for (int n = 0; n < nb_epochs; n++)
            {
                for (int i = 0; i < X.Count; i++)
                {
                    List<double> x = X[i];
                    List<double> y = Y[i];
					//Console.WriteLine("Lista sa brojem crnih "+x.Count);
					//Console.WriteLine("Lista sa brojem izlaza "+y.Count);

                    List<double> predicted = this.forward(x);
					//Console.WriteLine("Duzina predicted " + predicted.Count);
					...
					////////////dodavanje greske u listu
					Program.greskeZeljenaDobijena.Add(grad);
